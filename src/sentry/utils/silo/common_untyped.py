...
def apply_decorators(
    ...
    def is_module(src_path, module_name):
...
class Keywords:
    ...
    @classmethod
    def _parse_words(cls, name):
        return tuple(re.findall(".[a-z]*", name))

    @classmethod
    def _contains(cls, name, target):
...
def has_region_name(name, keywords):
    return keywords["region"].check(name)


def has_control_name(name, keywords):
